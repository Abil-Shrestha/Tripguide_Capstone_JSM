...
const BlogCardVertical = ({ blog }: IBlogCardHorizontalProps) => {
  ...
  return (
    <div className='col-span-12 lg:col-span-6 p-3 sm:p-6 bg-white dark:bg-cBlack-2 dark:border-cBlack-2 border border-gray-200 rounded-lg'>
      <div className='flex flex-col gap-2 w-full'>
        <div className='w-full'>
          <img src={blog.image} alt='blog' className='rounded-lg object-cover h-full w-full' />
        </div>
        <div className='flex flex-col justify-between w-full'>
          <div className='flex flex-col xs:flex-row gap-2 sm:gap-0 items-center justify-between mt-4'>
            <Link scroll={false} href='/'>
              <button
                type='button'
                className='border-2 border-[#f5f6fe] bg-[#f5f6fe] hover:bg-white hover:border-[#f5f6fe] dark:bg-cBlack-3
          dark:border-cBlack-3 text-[#878CFF] dark:hover:text-white px-6 py-3 rounded-md
          ...
          <div className='my-6 w-full'>
            <h3 className='text-[24px] lg:text-[28px] mb-4'>{blog.title}</h3>
