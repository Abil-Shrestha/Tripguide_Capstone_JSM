...
const Input = ({ label, icon, type = 'text', name, value, onChange, handleClick, styles, select, options, ...rest }: FormInputProps) => (
  <div className={`${icon ? 'relative' : ''} flex flex-col gap-2 w-full`}>
    ...
    { select ? (
      <select className={`rounded-lg bg-[#E7ECF3] dark:bg-cBlack-2 h-[48px] px-4 border border-gray-200 dark:border-cBlack-2 placeholder:text-cBlack-6 focus:outline-none dark:focus:border-cBlack-2 placeholder:text-sm ${styles}`}>
        {options?.map((option) => (
